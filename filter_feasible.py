...
def filter_timeout_instances():
    ...
    for xlsx_file in xlsx_files:
        ...
        try:
            ...
            # Filter rows with feasible status
            feasible_rows = df[df[status_col].astype(str).str.contains('FEASIBLE', case=False, na=False)]

            if len(feasible_rows) == 0:
                print(f"  No feasible instances found")
                continue
            ...
            output_file = f"{config_name}_feasible.txt"
...
def main():
    ...
    txt_files = glob.glob("*_feasible.txt")
    if txt_files:
        print(f"\nGenerated {len(txt_files)} feasible instance files:")
