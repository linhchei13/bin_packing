...
bins_used = []
instance_name = ""
config_name = "BPP_S_R_SB"
start = timeit.default_timer()
interrupt_handled = False  # Flag to prevent reentrant signal handling
...
def handle_interrupt(signum, frame):
    global best_bins, variables_length, clauses_length, instance_name, start, optimal_bins, interrupt_handled
    ...
    try:
        # Get the best bins found (or upper_bound if no solution found)
        current_bins = best_bins if best_bins > 0 else (best_bins if best_bins != float('inf') else upper_bound)
        ...
        # If any error occurs in the signal handler, just exit
        print("Error handling interrupt, exiting...")
        pass
    finally:
        sys.exit(124)  # Standard timeout exit code
...
def run_with_runlim(command, timeout, memory_limit):
    ...
    # Sử dụng shlex để xử lý command an toàn hơn
    import shlex
    ...
    process = None
    try:
        # Use shlex.split() thay vì split() đơn thuần
        cmd_args = shlex.split(runlim_cmd)
...
def display_solution_each_bin(W, H, rectangles, positions, rotations, bins_used, instance_name):
    ...
    for bin_idx, items_in_bin in enumerate(bins_used):
        ...
        for item_idx in items_in_bin:
            ...
            cx, cy = x0 + w/2, y0 + h/2
            rgb = matplotlib.colors.to_rgb(colors[item_idx])
    ...
    try:
        plt.savefig(f'{config_name}/{instance_name}_all_bins.png', dpi=150, bbox_inches='tight')
        plt.close()
...
def solve_single_instance(instance_name, timeout=900):
    ...
    try:
        ...
        write_excel_results(result_data, excel_file)
    ...
    for instance_name in instances_to_process:
        ...
        try:
            ...
            if os.path.exists('./runlim'):
                ...
                command = f"python3 {script_name} {instance_name}"
                    ...
                    try:
                        with open(result_file, 'r') as f:
                            ...
                            if content:  # Check if file is not empty
                                ...
                                logger.warning(f"Empty result file: {result_file}")
                                os.remove(result_file)  # Clean up empty file
                    except json.JSONDecodeError as e:
                        logger.error(f"Failed to parse JSON from {result_file}: {e}")
                        try:
                            os.remove(result_file)  # Clean up corrupt file
                            logger.info(f"Removed corrupt file: {result_file}")
                        except OSError:
                            pass
                    except Exception as e:
                    ...
                    try:
                        with open(checkpoint_file, 'r') as f:
                            ...
                            if content:  # Check if file is not empty
                                ...
                                logger.warning(f"Empty checkpoint file: {checkpoint_file}")
                                os.remove(checkpoint_file)  # Clean up empty file
                    except json.JSONDecodeError as e:
                        logger.error(f"Failed to parse JSON from {checkpoint_file}: {e}")
                        try:
                            os.remove(checkpoint_file)  # Clean up corrupt file
                            logger.info(f"Removed corrupt file: {checkpoint_file}")
                        except OSError:
                            pass
                    except Exception as e:
                ...
                result = solve_single_instance(instance_name, timeout=900)
                if result:
