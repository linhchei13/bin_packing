...
clauses_length = 0
upper_bound = 0
wcnf_file = ""  # Global variable to store the WCNF file name
...
def handle_interrupt(signum, frame):
    ...
    with open(f'results_BPP_MS_C1_{instance_id}.json', 'w') as f:
        json.dump(result, f)
...
# Create BPP_MS_C1 folder if it doesn't exist
if not os.path.exists('BPP_MS_C1'):
    os.makedirs('BPP_MS_C1')
...
def display_solution(bin_width, bin_height, rectangles, bins_assignment, positions, instance_name):
    ...
    fig.suptitle(f'BPP_MS_C1 - {instance_name} - {num_bins} bins', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(f'BPP_MS_C1/{instance_name}.png')
    plt.close()
...
def save_checkpoint(instance_id, variables, clauses, bins, status="IN_PROGRESS"):
    ...
    with open(f'checkpoint_BPP_MS_C1_{instance_id}.json', 'w') as f:
        json.dump(checkpoint, f)

def BPP_MaxSAT(rectangles, lower_bound, upper_bound, bin_width, bin_height):
    """Solve 2D Bin Packing using Max-SAT approach with tt-open-wbo-inc"""
    global variables_length, clauses_length, best_num_bins, best_solution, wcnf_file
    ...
    with tempfile.NamedTemporaryFile(delete=False, mode='w', suffix='.wcnf') as file:
        wcnf_file = file.name
        print(f"Writing to temporary file: {wcnf_file}")
        # Add comments for clarity
        file.write(f"c 2D Bin Packing Problem, Bin={bin_width}x{bin_height}, n={n_rectangles}\n")
        file.write(f"c Lower bound={lower_bound}, Upper bound={upper_bound}\n")
        
        # Define variables
        # x[i,b] - item i is assigned to bin b
        for i in range(n_rectangles):
            for b in range(upper_bound):
                variables[f"x{i + 1},{b + 1}"] = counter
                counter += 1
        
        # Position variables for items within bins
        for i in range(n_rectangles):
            for e in range(bin_width - rectangles[i][0] + 2):  # px[i,e] - item i at x-position e
                variables[f"px{i + 1},{e}"] = counter
                counter += 1
            for f in range(bin_height - rectangles[i][1] + 2):  # py[i,f] - item i at y-position f
                variables[f"py{i + 1},{f}"] = counter
                counter += 1
        
        # Relative position variables for non-overlapping constraints
        for i in range(n_rectangles):
            for j in range(n_rectangles):
                if i != j:
                    variables[f"lr{i + 1},{j + 1}"] = counter  # item i left of item j
                    counter += 1
                    variables[f"ud{i + 1},{j + 1}"] = counter  # item i under item j
                    counter += 1
        
        # Bin usage variables - b[k] means bin k is used (1-indexed)
        for b in range(1, upper_bound + 1):
            variables[f"b_{b}"] = counter
            counter += 1
        
        # Prepare hard clauses
        hard_clauses = []
        
        # Constraint 1: Each item must be assigned to exactly one bin
        for i in range(n_rectangles):
            clause = [variables[f"x{i + 1},{b + 1}"] for b in range(upper_bound)]
            hard_clauses.append(clause)
            
            # At most one bin (pairwise constraints)
            for b1 in range(upper_bound):
                for b2 in range(b1 + 1, upper_bound):
                    hard_clauses.append([-variables[f"x{i + 1},{b1 + 1}"], -variables[f"x{i + 1},{b2 + 1}"]])
        
        # Constraint 2: Order encoding for position variables
        for i in range(n_rectangles):
            # X-position order encoding
            for e in range(bin_width - rectangles[i][0] + 1):
                hard_clauses.append([-variables[f"px{i + 1},{e}"], variables[f"px{i + 1},{e + 1}"]])
            
            # Y-position order encoding
            for f in range(bin_height - rectangles[i][1] + 1):
                hard_clauses.append([-variables[f"py{i + 1},{f}"], variables[f"py{i + 1},{f + 1}"]])
        
        # Constraint 3: Bin usage ordering - if bin b is used, then all bins 1..b-1 must be used
        for b in range(2, upper_bound + 1):
            hard_clauses.append([-variables[f"b_{b}"], variables[f"b_{b-1}"]])
        
        # Constraint 4: Items assigned to bin b imply bin b is used
        for b in range(upper_bound):
            for i in range(n_rectangles):
                hard_clauses.append([-variables[f"x{i + 1},{b + 1}"], variables[f"b_{b + 1}"]])
        
        # Get max dimensions for symmetry breaking
        max_width = max([rect[0] for rect in rectangles])
        max_height = max([rect[1] for rect in rectangles])
        
        # Constraint 5: Non-overlapping constraints with C1 symmetry breaking
            ...
            if h1:
                for e in range(i_width):
                    if f"px{j + 1},{e}" in variables:
                        hard_clauses.append(bin_condition + [-variables[f"lr{i + 1},{j + 1}"], 
                                          -variables[f"px{j + 1},{e}"]])
            
            if h2:
                for e in range(j_width):
                    if f"px{i + 1},{e}" in variables:
                        hard_clauses.append(bin_condition + [-variables[f"lr{j + 1},{i + 1}"], 
                                          -variables[f"px{i + 1},{e}"]])
            
            if v1:
                for f in range(i_height):
                    if f"py{j + 1},{f}" in variables:
                        hard_clauses.append(bin_condition + [-variables[f"ud{i + 1},{j + 1}"], 
                                          -variables[f"py{j + 1},{f}"]])
            
            if v2:
                for f in range(j_height):
                    if f"py{i + 1},{f}" in variables:
                        hard_clauses.append(bin_condition + [-variables[f"ud{j + 1},{i + 1}"], 
        ...
        for i in range(n_rectangles):
            for bin_idx in range(upper_bound):
                # Item must fit horizontally
                hard_clauses.append([-variables[f"x{i + 1},{bin_idx + 1}"], 
                                   variables[f"px{i + 1},{bin_width - rectangles[i][0]}"]])
                # Item must fit vertically
                hard_clauses.append([-variables[f"x{i + 1},{bin_idx + 1}"], 
                                   variables[f"py{i + 1},{bin_height - rectangles[i][1]}"]])
        
        # Prepare soft clauses for bin minimization
        soft_clauses = []
        
        # We want to minimize the number of used bins
        for b in range(lower_bound + 1, upper_bound + 1):
            soft_clauses.append((1, [-variables[f"b_{b}"]]))  # Penalize using bin b (negative to minimize)
        
        # Require at least lower_bound bins to be used (ensures valid solution)
        if lower_bound > 0:
            hard_clauses.append([variables[f"b_{lower_bound}"]])
    ...
    try:
                ...
                if line.startswith('v '):
                    ...
                    if " " in binary_string:
                        try:
                            ...
                            # Try as space-separated binary values
                    ...
                    # Extract bin variables and find minimum bins used
                    bin_true_vars = []
                    for b in range(1, upper_bound + 1):
                        var_key = f"b_{b}"
                        if var_key in variables and variables[var_key] in true_vars:
                            bin_true_vars.append(b)
                    
                    if bin_true_vars:
                        optimal_bins = max(bin_true_vars)  # Highest bin used is the number of bins
                        print(f"Bins used: {sorted(bin_true_vars)}")
                        ...
                        print("WARNING: No bin variables are true! Using lower bound as fallback.")
                    ...
                    for i in range(n_rectangles):
                        ...
                        for f in range(bin_height - rectangles[i][1] + 1):
                            var_key = f"py{i + 1},{f}"
                            if var_key in variables and variables[var_key] in true_vars:
                                if f == 0 or f"py{i + 1},{f-1}" not in variables or variables[f"py{i + 1},{f-1}"] not in true_vars:
                                    positions[i][1] = f
                                    break
                
                # Filter out empty bins
                bins_assignment = [bin_items for bin_items in bins_assignment if bin_items]
                filtered_bins = [bin_items for bin_items in bins_assignment if bin_items]
                best_solution = (filtered_bins, positions)
        else:
        ...
        os.remove(wcnf_file)
        return optimal_bins, (bins_assignment, positions) if best_solution else None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        test_instances = range(1, len(instances))  # All instances from 1 to 100
        ...
        for instance_id in test_instances:
            ...
            if os.path.exists(f'results_BPP_MS_C1_{instance_id}.json'):
                os.remove(f'results_BPP_MS_C1_{instance_id}.json')
            
            # Run the instance with runlim
            command = f"./runlim -r {TIMEOUT} python3 BPP_MS_C1.py {instance_id}"
            try:
                ...
                if os.path.exists(f'results_BPP_MS_C1_{instance_id}.json'):
                    with open(f'results_BPP_MS_C1_{instance_id}.json', 'r') as f:
                        result = json.load(f)
                
                # If no results file, check checkpoint
                elif os.path.exists(f'checkpoint_BPP_MS_C1_{instance_id}.json'):
                    with open(f'checkpoint_BPP_MS_C1_{instance_id}.json', 'r') as f:
                        result = json.load(f)
            ...
            for file in [f'results_BPP_MS_C1_{instance_id}.json', f'checkpoint_BPP_MS_C1_{instance_id}.json']:
                if os.path.exists(file):
                    os.remove(file)
            
            # Clean up any temporary WCNF files that might be left behind
            import glob
    ...
    # Single instance mode
        ...
        try:
            ...
            # Ensure upper bound is at least lower bound
            if upper_bound != float('inf') and upper_bound < lower_bound:
            ...
            # Solve using Max-SAT
            optimal_bins, solution = BPP_MaxSAT(rectangles, lower_bound, upper_bound, bin_width, bin_height)
            ...
            final_bins = best_num_bins if best_num_bins != float('inf') else upper_bound
            ...
            result = {
                ...
                'Status': 'COMPLETE' if optimal_bins else 'ERROR'
            }
