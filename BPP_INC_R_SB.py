...
def handle_interrupt(signum, frame):
    ...
    with open(f'results_BPP_INC_R_SB_{instance_id}.json', 'w') as f:
        json.dump(result, f)
...
# Create BPP_INC_R_SB folder if it doesn't exist
if not os.path.exists('BPP_INC_R_SB'):
    os.makedirs('BPP_INC_R_SB')
...
def display_solution(bin_width, bin_height, rectangles, bins_assignment, positions, rotations, instance_name):
    ...
    ncols = min(num_bins, 4)
    nrows = (num_bins + ncols - 1) // ncols
    # Create subplots for each bin
    fig, axes = plt.subplots(nrows, ncols, figsize=(6 * ncols, 6 * nrows))
    fig.suptitle(f'BPP_INC_R_SB - {instance_name} solution', fontsize=16)
    ...
    for bin_idx, items_in_bin in enumerate(bins_assignment):
        ...
        ax.set_xlim(0, bin_width)
        ax.set_ylim(0, bin_height)
        ax.set_xticks(range(0, bin_width + 1, max(2, bin_width // 10)))
        ax.set_yticks(range(0, bin_height + 1, max(2, bin_height // 10)))
        ax.set_xlabel('width')
    ...
    plt.tight_layout()
    plt.savefig(f'BPP_INC_R_SB/{instance_name}.png')
    plt.close()
...
if os.path.exists('BPP_INC_R_SB_timeout.txt'):
            with open('BPP_INC_R_SB_timeout.txt', 'r') as f:
...
def save_checkpoint(instance_id, variables, clauses, num_bins, status="IN_PROGRESS"):
    ...
    with open(f'checkpoint_BPP_INC_R_SB_{instance_id}.json', 'w') as f:
...
def OPP(rectangles, max_bins, bin_width, bin_height):
    ...
    global optimal_solution, optimal_rotations, upper_bound
    cnf = CNF()
    variables = {}
    counter = 1
    print(max_bins)
    ...
    with Glucose4() as solver:
        ...
        while lb <= ub:
            ...
            if is_sat:
                ...
                optimal_solution = (used_bins, positions)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        for instance_id in range(1, len(instances)):
            ...
            for temp_file in [f'results_BPP_INC_R_SB_{instance_id}.json', f'checkpoint_BPP_INC_R_SB_{instance_id}.json']:
                if os.path.exists(temp_file):
                    os.remove(temp_file)
            
            # Run instance with timeout
            command = f"./runlim -r {TIMEOUT} python3 BPP_INC_R_SB.py {instance_id}"
            ...
            try:
                ...
                if os.path.exists(f'results_BPP_INC_R_SB_{instance_id}.json'):
                    with open(f'results_BPP_INC_R_SB_{instance_id}.json', 'r') as f:
                        result = json.load(f)
                elif os.path.exists(f'checkpoint_BPP_INC_R_SB_{instance_id}.json'):
                    with open(f'checkpoint_BPP_INC_R_SB_{instance_id}.json', 'r') as f:
                        result = json.load(f)
