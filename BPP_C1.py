...
def handle_interrupt(signum, frame):
    ...
    with open(f'results_BPP_C1_{instance_id}.json', 'w') as f:
        json.dump(result, f)
...
def display_solution(bin_width, bin_height, rectangles, bins_assignment, positions, instance_name):
    ...
    plt.tight_layout()
    plt.savefig(f'BPP_C1/{instance_name}.png')
    plt.close()
...
def save_checkpoint(instance_id, variables, clauses, num_bins, status="IN_PROGRESS"):
    ...
    with open(f'checkpoint_BPP_C1_{instance_id}.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    "CL_10_100_6", "CL_10_100_7", "CL_10_100_8", "CL_10_100_9", "CL_10_100_10"
        ...
        for instance_id in range(1, len(instances)):
            ...
            for temp_file in [f'results_BPP_C1_{instance_id}.json', f'checkpoint_BPP_C1_{instance_id}.json']:
                if os.path.exists(temp_file):
                    os.remove(temp_file)
            
            # Run instance with timeout
            command = f"./runlim -r {TIMEOUT} python3 BPP_C1.py {instance_id}"
            ...
            try:
                ...
                if os.path.exists(f'results_BPP_C1_{instance_id}.json'):
                    with open(f'results_BPP_C1_{instance_id}.json', 'r') as f:
                        result = json.load(f)
                elif os.path.exists(f'checkpoint_BPP_C1_{instance_id}.json'):
                    with open(f'checkpoint_BPP_C1_{instance_id}.json', 'r') as f:
                        result = json.load(f)
