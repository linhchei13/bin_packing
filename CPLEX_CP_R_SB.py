"""
2D Bin Packing Problem - Rotation Version with CP using CPLEX
...
def solve_bin_packing_with_rotation(W, H, rectangles, time_limit=900):
    """
    Solve 2D Bin Packing with rotation using CPLEX CP with C1 symmetry breaking and objective minimization
    ...
    # Create the CP model
    model = CpoModel(name="2D_BinPacking_Rotation_C1")
    ...
    # Variables: bin assignment, coordinates and rotation for each rectangle
    bin_assignment = [model.integer_var(0, max_bins-1, f'bin_{i}') for i in range(n)]
    x = {}
    y = {}
    rotate = [model.binary_var(f'rotate_{i}') for i in range(n)]  # 1 if rotated, 0 if not
    ...
    for i in range(n):
        for b in range(max_bins):
            x[i, b] = model.integer_var(0, W - min(rectangles[i][0], rectangles[i][1]), f'x_{i}_{b}')
            y[i, b] = model.integer_var(0, H - min(rectangles[i][0], rectangles[i][1]), f'y_{i}_{b}')
        
        # Calculate effective dimensions based on rotation
        w_orig, h_orig = rectangles[i]
        w_eff[i] = model.integer_var(min(w_orig, h_orig), max(w_orig, h_orig), f'w_eff_{i}')
        h_eff[i] = model.integer_var(min(w_orig, h_orig), max(w_orig, h_orig), f'h_eff_{i}')
        
        # Set effective dimensions based on rotation
        model.add(model.if_then(rotate[i] == 0, w_eff[i] == w_orig))
        model.add(model.if_then(rotate[i] == 0, h_eff[i] == h_orig))
        model.add(model.if_then(rotate[i] == 1, w_eff[i] == h_orig))
        model.add(model.if_then(rotate[i] == 1, h_eff[i] == w_orig))
        ...
        for j in range(i+1, n):
            for b in range(max_bins):
                ...
                model.add(model.logical_or([
                    bin_assignment[i] != b,
                    bin_assignment[j] != b,
                    x[i, b] + w_eff[i] <= x[j, b],  # i is to the left of j
                    x[j, b] + w_eff[j] <= x[i, b],  # j is to the left of i
                    y[i, b] + h_eff[i] <= y[j, b],  # i is below j
                    y[j, b] + h_eff[j] <= y[i, b]   # j is below i
                ]))
                ...
                if min_width_i + min_width_j > W:
                    # If two rectangles can't fit side by side even with rotation, disable horizontal placements
                    model.add(model.logical_or([
                        bin_assignment[i] != b,
                        bin_assignment[j] != b,
                        y[i, b] + h_eff[i] <= y[j, b],  # i is below j
                ...
                if min_height_i + min_height_j > H:
                    # If two rectangles can't fit vertically even with rotation, disable vertical placements
                    model.add(model.logical_or([
                        bin_assignment[i] != b,
                        bin_assignment[j] != b,
                        x[i, b] + w_eff[i] <= x[j, b],  # i is to the left of j
    ...
    # C1 Symmetry Breaking - For equivalent rectangles (considering rotation), enforce ordering
    for i in range(n):
        for j in range(i+1, n):
            w_i, h_i = rectangles[i]
            w_j, h_j = rectangles[j]
            ...
            if equivalent:
                # If rectangles are equivalent, enforce bin ordering
...
def display_solution(W, H, rectangles, positions, assignments, rotations, instance_name):
    ...
    plt.savefig(f'CPLEX_CP_R_SB/{instance_name}.png', dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        # Set time limit for runlim
        TIME_LIMIT = 900  # 10 minutes
