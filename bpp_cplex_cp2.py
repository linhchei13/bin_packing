...
def main_solver(file_path, time_limit):
    ...
    width_vars = [model.integer_var(0, max(data['size_item'][i][0], data['size_item'][i][1]), f"width_{i}") for i in range(n)]
    height_vars = [model.integer_var(0, max(data['size_item'][i][0], data['size_item'][i][1]), f"height_{i}") for i in range(n)]
    
    # Add constraints to set width and height based on rotation
    for i in range(n):
        w = data['size_item'][i][0]
        h = data['size_item'][i][1]
        
        # If not rotated: width = w, height = h
        # If rotated: width = h, height = w
        model.add(model.logical_or([
            model.logical_and([rot_vars[i] == 0, width_vars[i] == w, height_vars[i] == h]),
            model.logical_and([rot_vars[i] == 1, width_vars[i] == h, height_vars[i] == w])
        ]))
    
    # Ensure items stay within bin boundaries
    for i in range(n):
        model.add(x_vars[i] + width_vars[i] <= W)
        model.add(y_vars[i] + height_vars[i] <= H)
    ...
    for i in range(n-1):
        for j in range(i+1, n):
            ...
            no_overlap = model.logical_or([
                x_vars[i] + width_vars[i] <= x_vars[j],  # i is to the left of j
                x_vars[j] + width_vars[j] <= x_vars[i],  # j is to the left of i
                y_vars[i] + height_vars[i] <= y_vars[j],  # i is below j
                y_vars[j] + height_vars[j] <= y_vars[i],  # j is below i
            ])
    ...
    for j in range(1, k):
        model.add(model.logical_or([
            bin_used[j-1] == 1,  # Bin j-1 is used
            bin_used[j] == 0     # Bin j is not used
        ]))
    ...
    model.write_information('model_cplex.lp')
    # Solve model
    print('--------------Solving--------------')
    start = time.time()
    ...
    if solution:
        ...
        widths = [solution.get_value(width_vars[i]) for i in range(n)]
        heights = [solution.get_value(height_vars[i]) for i in range(n)]
