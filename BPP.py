...
def handle_interrupt(signum, frame):
    ...
    with open(f'results_BPP_{instance_id}.json', 'w') as f:
        json.dump(result, f)
...
signal.signal(signal.SIGALRM, handle_interrupt)  # Alarm signal for timeouts
...
def display_solution(bin_width, bin_height, rectangles, bins_assignment, positions, instance_name):
    ...
    fig.suptitle(f'BPP - {instance_name} solution', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(f'BPP/{instance_name}.png')
    plt.close()
...
if os.path.exists("BPP_timeout.txt"):
    with open("BPP_timeout.txt", 'r') as f:
...
def save_checkpoint(instance_id, variables, clauses, num_bins, status="IN_PROGRESS"):
    ...
    with open(f'checkpoint_BPP_{instance_id}.json', 'w') as f:
...
def OPP(rectangles, max_bins, bin_width, bin_height):
    ...
    # Constraint 3: Non-overlapping constraints
    def add_non_overlapping(i, j, bin_idx):
        """Add non-overlapping constraints for items i and j in bin bin_idx"""
        ...
        # At least one relative position must be true if both items are in same bin
        ...
        # If i is left of j: px[j,e] = false for e < width[i]
        for e in range(i_width):
            if f"px{j + 1},{e}" in variables:
                cnf.append(bin_condition + [-variables[f"lr{i + 1},{j + 1}"], 
                          -variables[f"px{j + 1},{e}"]])
        
        # If j is left of i: px[i,e] = false for e < width[j]
...
def BPP(lower, upper, bin_width, bin_height, rectangles):
    ...
    return optimal_bins, optimal_solution
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        for instance_id in range(1, len(instances)):
            ...
            for temp_file in [f'results_BPP_{instance_id}.json', f'checkpoint_BPP_{instance_id}.json']:
                if os.path.exists(temp_file):
                    os.remove(temp_file)
            
            # Run instance with runlim
            command = f"./runlim -r {TIMEOUT} python3 BPP.py {instance_id}"
            ...
            try:
                ...
                if os.path.exists(f'results_BPP_{instance_id}.json'):
                    with open(f'results_BPP_{instance_id}.json', 'r') as f:
                        result = json.load(f)
                elif os.path.exists(f'checkpoint_BPP_{instance_id}.json'):
                    with open(f'checkpoint_BPP_{instance_id}.json', 'r') as f:
                        result = json.load(f)
    ...
    # Single instance mode
        ...
        try:
            ...
            # Solve
            optimal_bins, solution = BPP(lower_bound, upper_bound, bin_width, bin_height, rectangles)
